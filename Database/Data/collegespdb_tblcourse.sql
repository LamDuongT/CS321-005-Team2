...
CREATE TABLE `tblcourse` (
  ...
  `courseDesc` mediumtext,
  `creditHours` int(11) DEFAULT NULL,
  `catalogID` int(11) NOT NULL,
  `majorID` varchar(50) DEFAULT NULL,
  `minorID` varchar(50) DEFAULT NULL,
  PRIMARY KEY (`courseID`),
  KEY `course_catalog_idx` (`catalogID`),
  KEY `course_major_idx` (`majorID`),
  KEY `course_minor_idx` (`minorID`),
  CONSTRAINT `course_catalog` FOREIGN KEY (`catalogID`) REFERENCES `tblcatalog` (`catalogID`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB AUTO_INCREMENT=179 DEFAULT CHARSET=utf8 COMMENT='Courses	';
...
-- Dump completed on 2018-04-24 21:44:11
